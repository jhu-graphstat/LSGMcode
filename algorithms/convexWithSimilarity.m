function [P,Pp]=convexWithSimilarity(A,B,seeds,similarity, similarityWeight, roundGrad,varargin)
...
while (f > tol) && (var > tol2) && (Pdiff>tol) 
    ...
    grad(nonSeeds, nonSeeds) = (1-similarityWeight)*grad(nonSeeds, nonSeeds) + similarityWeight*similarity;
    ...
    aq = trace(C*C');
    bq = trace(C*D'+D*C');
    cq = trace(similarity*(P-Ps));
    aopt = -((1-similarityWeight)*bq + similarityWeight*cq)/(2*(1-similiarityWeight)*aq);
