...
void partition(MTRand *R,Node ***node, GreedyBase *greedy, bool silent, double lambda);
void repeated_partition(MTRand *R, Node ***node, GreedyBase *greedy, bool silent,int Ntrials, double lambda);
...
int main(int argc,char *argv[]){
  ...
  if( argc != 5 ){
    cout << "Call: ./infomap <seed> <network.net> <# attempts> <lambda>" << endl;
    exit(-1);
  }
  
  // sshum: adding a lambda value to weight
  ...
  // Initiation
  GreedyBase* greedy;
  greedy = new Greedy(R,Nnode,totalDegree,node,lambda);
  greedy->initiate();
  
  double uncompressedCodeLength = -greedy->nodeDegree_log_nodeDegree;

  cout << "Now partition the network:" << endl;
  repeated_partition(R,&node,greedy,false,Ntrials,lambda);
    ...
    if((iteration > 0) && (iteration % 2 == 0) && (greedy->Nnode > 1)){  // Partition the partition
      ...
      for(int i=0;i<greedy->Nnode;i++){
        ...
        if(sub_Nnode > 1){
          ...
          sub_greedy = new Greedy(R,sub_Nnode,totalDegree,sub_node,lambda);
          sub_greedy->initiate();
          partition(R,&sub_node,sub_greedy,true,lambda);
...
void repeated_partition(MTRand *R, Node ***node, GreedyBase *greedy, bool silent,int Ntrials,double lambda){
  ...
  for(int trial = 0; trial<Ntrials;trial++){
    ...
    partition(R,&cpy_node,greedy,silent,lambda);
