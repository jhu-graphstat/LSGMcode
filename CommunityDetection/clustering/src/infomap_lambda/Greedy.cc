...
Greedy::Greedy(MTRand *RR, int nnode, double deg, Node **ah, double lambda1){
    ...
    for (int j=0; j<NmodLinks; j++) {
      ...
      if(toM != fromM){
				...
				//double deltaL = delta_exit - 2.0*delta_exit_log_exit + delta_degree_log_degree;
                                double deltaL = delta_exit - (1.0 + lambda) * delta_exit_log_exit + lambda * delta_degree_log_degree;
